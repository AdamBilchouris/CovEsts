...
#' @param kernel The name of the kernel function to be used. Possible values are:
...
compute_rho_T1 <- function(x, meanX, T1, h, Xij_mat, kernel="gaussian", kernel_params=c(), custom_kernel = FALSE) {
...
compute_truncated <- function(X, x, meanX, t, T1, T2, h, kernel="gaussian",  kernel_params=c(), custom_kernel = FALSE, pd = TRUE) {
  ...
  vals_truncated_1 <- c()
  for(i in 1:length(t)) {
    vals_truncated_1 <- c(vals_truncated_1, compute_truncated_point(x, meanX, t[i], T1, T2, h, Xij_mat, rho_T1, kernel, kernel_params, custom_kernel))
  }
...
compute_adjusted <- function(X, x, meanX, t, h, kernel="gaussian", kernel_params=c(), custom_kernel = FALSE, pd = TRUE) {
